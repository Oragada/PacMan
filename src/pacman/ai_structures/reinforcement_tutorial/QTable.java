package pacman.ai_structures.reinforcement_tutorial;
...
/**
    ...
    public class QTable {
        ...
        int getBestAction(char[] map){
        	float[] actions = getActionsQValues(map);
        	float highest = -1f;
        	int highDir = -1;
        ...
        void updateQvalue(int reward, char[] map){
        	int bestA = getBestAction(map);
        	float[] m = table.get(getMapString(map));
        	float maxQa = gammaValue*m[bestA];
